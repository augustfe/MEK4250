...
def solve_for_coeffs(N: int, f: sp.Expr = sp.sin(k * x * pi)) -> list[sp.Expr]:
    A = assemble_A(N)
    b = assemble_b(N, f)
    coeffs = A.solve(b)
    return coeffs
...
def true_u(x: float) -> float:
    return sp.sin(sp.pi * k * x) / (sp.pi * k) ** 2


def compute_L2_error(N: int) -> sp.Expr:
...
if __name__ == "__main__":
    ...
    savedir = savedir = Path(__file__).parents[1] / "figs"
    ...
    u = sp.lambdify(x, true_u(x), modules="numpy")
    x_vals = np.linspace(0, 1, 1000)

    plt.plot(x_vals, u(x_vals), label="True $u$")
    plt.plot(x_vals, u_N(x_vals), label=f"$u_{{{N}}}$")
    plt.legend()
    plt.title(f"Approximation of $u$ with $N = {N}$, $k = {k}$")
    plt.xlabel("$x$")
    plt.ylabel("$u$")
    plt.savefig(savedir / f"exercise_1_3_k={k}.pdf", bbox_inches="tight")
    plt.clf()
    ...
    plt.xlabel("$N$")
    plt.ylabel("Error")
    plt.title(f"Convergence of the error, $k = {k}$")
    plt.savefig(savedir / f"exercise_1_3_convergence_k={k}.pdf", bbox_inches="tight")
    plt.clf()

    errors = [error.evalf() for error in errors]
    print(errors)
